	...
	onload: function (listview) {
		...
		if (isVendor && !isAdmin) {
			...
			setTimeout(() => {
				...
				listview.filter_area.add([[ "Hotpot Meal", "vendor_id", "=", frappe.session.user ]]);
				listview.filter_area.add([[ "Hotpot Meal", "is_deleted", "=", 0 ]]);
			}, 50);
		}
		if (!isAdmin) {
		...
		listview.columns = listview.columns.filter(
			(column) => column.df && allowedFields.includes(column.df.fieldname)
		);
