...
def send_approval_request_email(to_email, user_data, request_data, doc, meal_name):
	email_subject = (
		f"Approval Request from {user_data.full_name} ({user_data.name}) for {request_data['request_type']}"
	)
	context = {
		"user_data": user_data,
					...
					if user_doc_admin_hr.fcm_token:
						try:
							...
							frappe.log_error(
								...
								f"Failed to send approval notification notification to user {user_doc_admin_hr.name}",
							)
