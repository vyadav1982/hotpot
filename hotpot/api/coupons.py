...
def generate_coupon():
	try:
		...
		# Retrieve the user document either by tag_id or email
		...
		# Block coupon generation on holidays or Sunday (unless allowed)
		if has_any_of_role(["Hotpot User", "Hotpot Admin", "Hotpot HR"]):
		...
		# Iterate through each meal and quantity
		for _j in range(int(qty)):
			for i in range(len(meal_ids)):
				...
				# Birthday and joining day check
				...
				# Check meal's max limit not exceeded
				...
				# Approval document validations (if present)
				if approval_id:
				...
				# End time check for same day meal
				if (
				...
				# Buffer check for same-day meals
				...
				# Update remaining count and approval status
...
def search_coupon(start_date, end_date, identifier):
	try:
		...
		# Get user doc based on identifier
