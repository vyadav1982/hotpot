...
def get_coupon_count(start_date, end_date):
	try:
		...
		if not has_any_of_role(["Hotpot User", "Hotpot Admin", "Hotpot HR","Hotpot Vendor"]):
		...
		if has_any_of_role(["Hotpot User", "Hotpot Admin", "Hotpot HR"]):
			...
			set_response(200, True, "Coupon data fetched successfully", response)
			return
			...
			transaction_doc.update(
				{
					...
					"type": "Credit",
					"message": f"{int(coupon_found.coupon_weight)} tokens credited to your wallet for '{meal_doc.meal_title}' meal cancellation for {(coupon_found.coupon_date).strftime('%d %b %Y')}.",
...
def get_all_coupons(
	...
	try:
		...
		frappe.db.rollback()
		frappe.log_error(frappe.get_traceback(), "Getting All Coupon Error")
		set_response(500, False, f"Server error: {str(e)}")
		return
...
def generate_coupon():
	try:
		...
		for _j in range(int(qty)):
			for i in range(len(meal_ids)):
				...
				try:
					...
					mcategory_capitalize = mcategory.title()
					transaction_doc = frappe.new_doc("Hotpot Transaction History")
					transaction_doc.update(
						{
							"employee_id": user_doc.get("name"),
		...
		for doc in temp_docs:
			doc.insert()
		frappe.db.commit()
		message = f"{mcategory_capitalize} booked for {from_date.strftime('%d %b %Y')}."
		if for_guest and role in ["Hotpot User", "Hotpot HR"]:
...
def generate_for_all(docname):
	try:
		...
		if not meal_doc:
			return {"status": "error", "msg": "Meal not found", "errors": []}
		...
		for users in user_docs:
			...
			exists = frappe.db.sql(query, params)
			if exists:
				errors.append(f"Coupon already exists for {users.get('full_name')} on {start_date.strftime("%d %b").lstrip("0")}")
				continue
		...
		if errors:
			return {
				"status": "error",
				"msg": "Some coupons could not be generated",
				"errors": errors,
			}
		...
		for users in user_docs:
			try:
				if users.get("fcm_token"):
					...
					meal_title = meal_title[0].upper() + meal_title[1:]
					messages = [
						f"👨‍💼 Admin has generated your {meal_title} coupon. Please check your account! 🎉",
						f"✅ Your {meal_title} coupon has been created by Admin. Ready to use! 🍽️",
						f"📢 Good news! Admin issued your {meal_title} coupon successfully.",
						f"🥳 Admin generated a {meal_title} coupon for you. Enjoy your meal!",
						f"🍴 Your {meal_title} coupon is now available — generated by Admin.",
						f"✨ Admin successfully created your {meal_title} coupon. Bon appétit!",
						f"🔥 {meal_title} coupon generated by Admin. Time to fuel up!",
						f"💯 Success! Admin has created your {meal_title} coupon.",
						f"🎊 Admin booked your {meal_title} coupon. Check it out in your account!",
						f"👌 Done! {meal_title} coupon issued by Admin. Enjoy the treat!"
					]
					...
					send_notification_by_token(
						...
						"🍽️ Meal Booked✅",
						chosen_message,
						date=get_local_datetime_obj(meal_doc.get("meal_date")).date(),
						doc_id=None,
						text="coupons",
					)
			except Exception:
		...
		frappe.db.rollback()
		frappe.log_error(frappe.get_traceback(), "generate_for_all failed")

		return {
			"status": "error",
			"msg": "Unexpected error occurred while generating coupons",
			"errors": [str(e)],
		}
