...
function normalizeRoute(route) {
    ...
    if (route.toLowerCase().startsWith("workspaces/")) {
        ...
        return workspaceName.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '');
    }
...
function getDefaultWorkspace() {
    ...
    if (roles.includes("Hotpot Admin")) return "hotpot-workspace";
    if (roles.includes("Hotpot HR")) return "hotpot-hr-workspace";
    if (roles.includes("Hotpot Finance")) return "hotpot-finance-workspace";
    if (roles.includes("Hotpot Vendor")) return "hotpot-vendor-workspace";
    return "home";
}
...
frappe.set_route = function (...args) {
    try {
        const target = args.length > 1 ? args.join('/') : 
